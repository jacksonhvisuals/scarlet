    ...
    if (workRequest.readyState === 4) {
        if (workRequest.status === 200 || workRequest.status == 0) {
            ...
            JSON.parse(workRequest.responseText).forEach(function (work) {
                ...
                html += "<div class=\"col s12 m6 l4\"><div class=\"card hoverable\"><div class=\"card-image waves-effect waves-block waves-light\"><img src=" + work.image + "></div><div class=\"card-content\"><span class=\"card-title grey-text text-darken-4\">" + work.name + "<a href=" + work.link + " target=\"_blank\" style=\"color: #000000\"><i class=\"material-icons right\">launch</i></a></span><p><span>" + work.description + "</span></p></div><div class=\"card-reveal\"><span class=\"card-title grey-text text-darken-4\">" + work.name + "<i class=\"material-icons right\">close</i></span><p></p></div></div></div>";
            });
            work.innerHTML = html;
        }
    }
